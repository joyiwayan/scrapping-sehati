...
for p in range(0, len(selectOptProv) - 1, 1):
   ...
   if amount_pagination_current_page == 12:
      ...
      for x in range(int(last_page)):
         ...
         for y in range(lostPage, int(last_page)):
            time.sleep(4)
            driver.implicitly_wait(60)
            ...
            if page_number != 1 and page_number % 10 != 0:
                try:
                    btn_page_click = WebDriverWait(driver, 50).until(
                        ec.element_to_be_clickable((By.XPATH, f"//table[@id='GridView3']/tbody/tr[@class='GridPager']/td/table/tbody/tr/td/a[text() = '{page_number}']"))
                    )
                    driver.execute_script("arguments[0].click();", btn_page_click)
                    print(f"Clicked pagination button: {page_number}")
                except TimeoutException:
                    print(f"Timeout while clicking pagination button: {page_number}. Retrying...")
                    continue  # Retry logic can be implemented here
            elif page_number == 1:
                clickDetailPerRow()
            elif page_number % 10 == 0:
                try:
                    btn_page_next_click = WebDriverWait(driver, 50).until(
                        ec.element_to_be_clickable((By.XPATH, f"//table[@id='GridView3']/tbody/tr[@class='GridPager']/td/table/tbody/tr/td/a[contains(@href,'{page_number}')]"))
                    )
                    driver.execute_script("arguments[0].click();", btn_page_next_click)
                    print(f"Clicked next pagination button: {page_number}")
                except TimeoutException:
                    print(f"Timeout while clicking next pagination button: {page_number}. Retrying...")
                    continue  # Retry logic can be implemented here

            # Call to process the details per row
            clickDetailPerRow()
